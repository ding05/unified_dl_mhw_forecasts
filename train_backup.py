...
node_feat_filename = 'noaa_node_feats_eighth.npy'
adj_filename = 'noaa_adj_mat_eighth_0.6.npy'
...
node_feat_grid_normalized = (node_feat_grid - np.min(node_feat_grid[:,:376])) / (np.max(node_feat_grid[:,:376]) - np.min(node_feat_grid[:,:376]))
...
if lead_time == 1:
    ...
    train_graph_list = graph_list[:364]
    val_graph_list = graph_list[364:-1]
    test_graph_list = graph_list[364:-1]
    
    test_node_feats = node_feat_grid_normalized[:, 364 + window_size + lead_time - 1:-1]
    
    # Compute the percentiles using the training set only.
    node_feats_90 = np.percentile(node_feat_grid[:, :364], 90, axis=1)
    node_feats_95 = np.percentile(node_feat_grid[:, :364], 95, axis=1)
    node_feats_normalized_90 = np.percentile(node_feat_grid_normalized[:, :364], 90, axis=1)
    node_feats_normalized_95 = np.percentile(node_feat_grid_normalized[:, :364], 95, axis=1)
